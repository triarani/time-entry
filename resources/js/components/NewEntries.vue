<template>
  ...
  <div class="max-w-[750px] mx-auto p-4 bg-white dark:bg-gray-800 rounded shadow">
    <h2 class="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-200">New Time Entry</h2>
<form @submit.prevent="submitAll" @keydown.enter.prevent="addRow" @keydown.ctrl.s.prevent="submitAll" class="space-y-4">
      <div>
...
onMounted(fetchOptions);
  ...
  // rows for batch entry; start with a single empty row
  ...
  function addRow() {
    ...
    rows.value.push({ ...last }); // duplicate previous row values
  }
  ...
  async function submitAll() {
    ...
    try {
      ...
      // Expect error format { errors: { "entries.0.field": ["msg"] } }
